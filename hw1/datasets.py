...
def random_labelled_image(
    ...
    # Generate a random image with the given shape and value range
    image = torch.randint(low=low, high=high, size=shape, dtype=dtype)
    
    # Generate a random label within the range of [0, num_classes-1]
...
def torch_temporary_seed(seed: int):
    ...
    try:
        ...
        # Restore the previous random number generator state
...
class ImageStreamDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        # ====== YOUR CODE: ======
        while True:
            image = torch.randn(*self.image_dim)  # Generate a random noise image
